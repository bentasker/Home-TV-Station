...
do
    ...
    then    
        curl -s "http://127.0.0.1:8080/control/redirect/subscriber?app=${RTMP_APPLICATION}&name=${RTMP_STREAMNAME}.tmp&newname=${RTMP_STREAMNAME}"    
    fi
    ...
    then
        ...
        curl -s "http://127.0.0.1:8080/control/redirect/subscriber?app=${RTMP_APPLICATION}&name=${RTMP_STREAMNAME}&newname=${RTMP_STREAMNAME}.tmp"
    fi    
done
